...
public class Session_ME : ISession
{
	...
	private class MessageCollector
	{
		...
		private Message readMessage2(sbyte cmd)
		{
			...
			byte[] src = new byte[num4];
			int totalRead = 0;
			while (totalRead < num4)
			{
				int bytesRead = dataStream.Read(src, totalRead, num4 - totalRead);
				if (bytesRead == 0)
				{
					return null;
				}
		...
		private Message readMessage()
		{
			try
			{
				...
				while (totalRead < num)
				{
					int bytesRead = dataStream.Read(src, totalRead, num - totalRead);
					if (bytesRead == 0)
					{
						return null;
					}
