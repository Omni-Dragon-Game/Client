...
public class Image
{
	...
	public void getRGB(ref int[] data, int x1, int x2, int x, int y, int w, int h)
	{
		try
		{
			...
			texture2D.ReadPixels(new Rect(x, this.h - 1 - y, w, h), 0, 0);
			texture2D.Apply();
			RenderTexture.active = active;
			RenderTexture.ReleaseTemporary(temporary);
			Color[] pixels2 = texture2D.GetPixels();
			UnityEngine.Object.Destroy(texture2D);
			for (int j = 0; j < pixels2.Length && j < data.Length; j++)
			{
				data[j] = mGraphics.getIntByColor(pixels2[j]);
			}
		}
		catch (Exception ex)
		{
			Debug.LogWarning("getRGB safe fallback: " + ex.Message);
		}
	}
}
